...
function select_infos_table_corps_enseignant($con, $offset){
    $offend = $offset + 10;
    $requete = $con->prepare("SELECT u.first_name, u.last_name,m.name u.id AS module, m.hours_count FROM instructor i JOIN user u ON i.user_id =u.id JOIN instructor_module im ON im.instructor_id = i.id JOIN module m ON im.module_id = m.id LIMIT :offsetend OFFSET :offset");
    $requete->bindValue(':offsetend', (int) $offend, PDO::PARAM_INT);
    $requete->bindValue(':offset', (int) $offset, PDO::PARAM_INT);
    $requete->bindParam(':id', $id);
    $requete->execute();
